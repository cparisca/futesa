...
class ResCompanyInherit(models.Model):
    ...
    digital_certificate = fields.Text(
        string="Certificado digital público",  default="0"
    )
    software_identification_code = fields.Char(
        string="Código de identificación del software",  default="0"
    )
    identificador_set_pruebas = fields.Char(
        string="Identificador del SET de pruebas")
    software_pin = fields.Char(string="PIN del software",  default="0")
    password_environment = fields.Char(
        string="Clave de ambiente",  default="0"
    )
    seed_code = fields.Integer(
        string="Código de semilla",  default=5000000
    )
    issuer_name = fields.Char(
        string="Ente emisor del certificado",  default="0"
    )
    serial_number = fields.Char(
        string="Serial del certificado",  default="0"
    )
    document_repository = fields.Char(
        string="Ruta de almacenamiento de archivos",  default="0"
    )
    in_use_dian_sequence = fields.Selection(
        "_get_dian_sequence", "Secuenciador DIAN a utilizar", required=False
    )
    certificate_key = fields.Char(
        string="Clave del certificado P12",  default="0"
    )
    operation_type = fields.Selection(
        OPERATION_TYPE, string="Tipo de operación DIAN", 
    )
    pem = fields.Char(
        string="Nombre del archivo PEM del certificado",  default="0"
    )
    pem_file = fields.Binary("Archivo PEM")
    certificate = fields.Char(
        string="Nombre del archivo del certificado",  default="0"
    )
    ...
    date_init_contingency_4 = fields.Datetime(
        string="Fecha de inicio de contingencia 4"
    )
        ...
        if self.production:
            try:
                ...
                raise ValidationError(
                    _(
                        "No existe comunicación con la DIAN para el servicio de consulta de rangos de numeración"
                    )
                )
        else:
            try:
    ...
    def _old_generate_SignatureValue_GetNumberingRange(
        self,
        document_repository,
        password,
        data_xml_SignedInfo_generate,
        archivo_pem,
        archivo_certificado,
    ):
        ...
        try:
            ...
            raise ValidationError(
                _("Firma para el GestStatus no fué validada exitosamente")
            )
    ...
    def _generate_SignatureValue_GetNumberingRange(self, data_xml_SignedInfo_generate):
        ...
        try:
            ...
            key = crypto.load_pkcs12(archivo_key, password)
        except Exception as ex:
            raise UserError(tools.ustr(ex))
        try:
            signature = crypto.sign(
