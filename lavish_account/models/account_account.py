...
class account_tax(models.Model):
    ...
    #------------------------------------------------------------------------------------------------------------------
    #--------------------- SE HEREDA EL METODO ORIGINAL DE ODOO PARA AGREGAR LÓGICA lavish -------------------------------
    #------------------------------------------------------------------------------------------------------------------
    def compute_all(self, price_unit, currency=None, quantity=1.0, product=None, partner=None, is_refund=False, handle_price_include=True, include_caba_tags=False):
        ...
        sum_tax_amount,sum_tax_amount_selected = 0,[] #Variables lavish
        for tax in taxes:
            price_include = self._context.get('force_price_include', tax.price_include)

            if price_include or tax.is_base_affected:
                if tax.is_base_affected_only_taxes: # Lógica lavish
                    tax_base_amount = sum_tax_amount
                elif tax.base_affected_only_taxes_id:
                    tax_base_amount = 0
                    for tax_selected in sum_tax_amount_selected:
                        tax_base_amount += tax_selected[1] if tax_selected[0] == tax.base_affected_only_taxes_id.name else 0 # Fin Lógica lavish
                else:
            ...
            sum_tax_amount += tax_amount  # Lógica lavish
            sum_tax_amount_selected.append((tax.name,tax_amount)) # Lógica lavish
