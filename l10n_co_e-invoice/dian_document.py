...
class DianDocument(models.Model):
    ...
    def _get_dian_constants(self, data_header_doc):
        ...
        dian_constants["SoftwareSecurityCode"] = self._generate_software_security_code(
            ...
            data_header_doc.name) # Código de seguridad del software: (hashlib.new('sha384', str(self.company_id.software_id) + str(self.company_id.software_pin)))
        ...
        if partner.city_id:
            dian_constants["SupplierCityCode"] = partner.city_id.code_dian  # Municipio tabla 6.4.3 res.country.state.city
        ...
        if partner.city_id:
            dian_constants["SupplierCountrySubentityCode"] = partner.city_id.state_id.code  # Ciudad o departamento tabla 6.4.2 res.country.state
        dian_constants[
            "SupplierCountryCode"
    ...
    def _generate_data_constants_document(
        ...
        if partner_to_send.city_id:
            data_constants_document["CustomerCityCode"] = partner_to_send.city_id.code_dian  # Municipio tabla 6.4.3 res.country.state.city
...
<rep:Document>%(Document)s</rep:Document>
    ...
    def _generate_signature_signingtime(self):
        fmt = "%Y-%m-%dT%H:%M:%S"
        now_utc = self.document_id.fecha_xml#(timezone("UTC")) #datetime.now(timezone("UTC"))
        now_bogota = now_utc
