...
class hr_payroll_social_security(models.Model):
    ...
    def get_plano(self):
        ...
        for item in details:
            ...
            cTipIdTercero = switch_cTipIdTercero.get(item.employee_id.address_home_id.document_type, '/')
            if cTipIdTercero == '/':
            ...
            cCodUbiLaboral = right('00000'+entity_ccf.partner_id.city_id.code,5) if entity_ccf and entity_ccf.partner_id.city_id.code else right('00000'+item.employee_id.address_home_id.city_id.code,5)            
            #Obtener nombre
            ...
            cPrimerApellido = left(cPrimerApellido+' '*20,20) if not item.employee_id.address_home_id.first_lastname else left(item.employee_id.address_home_id.first_lastname+' '*20,20)
            cSegundoApellido = left(cSegundoApellido+' '*30,30) if not item.employee_id.address_home_id.second_lastname else left(item.employee_id.address_home_id.second_lastname+' '*30,30)
            cPrimerNombre = left(cPrimerNombre+' '*20,20) if not item.employee_id.address_home_id.firs_name else left(item.employee_id.address_home_id.firs_name+' '*20,20)
            cSegundoNombre = left(cSegundoNombre+' '*30,30) if not item.employee_id.address_home_id.second_name else left(item.employee_id.address_home_id.second_name+' '*30,30) 
        ...
        self.write({
            'txt_file': base64.b64encode(content_txt).decode('utf-8'),
