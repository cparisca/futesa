...
class x_reports(models.Model):
    _name = 'lavish.reports'
    _description = 'Reportes creados por lavish'
    ...
    def get_excel(self):        
        if self.query and self.columns:
            ...
            action = {
                        ...
                        'url': "web/content/?model=lavish.reports&id=" + str(self.id) + "&filename_field=excel_file_name&field=excel_file&download=true&filename=" + self.excel_file_name,
                        'target': 'self',
                    }
            return action
